...
CREATE OR REPLACE FUNCTION public.gamification_leaderboard(_from date, _to date)
 RETURNS TABLE(user_id uuid, full_name text, last_name text, photo_url text, "position" text, present_days integer, forms_count integer, streak_weeks integer, convocations integer, test_top5_count integer, test_pos_top3_count integer, vote_wins integer, vote_seconds integer, vote_thirds integer, votes_cast integer, points integer)
 LANGUAGE sql
...
tally_ranked AS (
  SELECT t.week_start, t.nominee_id,
         dense_rank() OVER (PARTITION BY t.week_start ORDER BY t.votes DESC) AS dr
  FROM tally t
),
podium AS (
  SELECT tr.nominee_id AS user_id,
         COUNT(*) FILTER (WHERE tr.dr = 1)::int AS first_c,
         COUNT(*) FILTER (WHERE tr.dr = 2)::int AS second_c,
         COUNT(*) FILTER (WHERE tr.dr = 3)::int AS third_c
  FROM tally_ranked tr
  WHERE tr.dr <= 3
  GROUP BY tr.nominee_id
),
cast_votes AS (
...
SELECT a.id,
       ...
       COALESCE(podium.first_c, 0),
       COALESCE(podium.second_c, 0),
       COALESCE(podium.third_c, 0),
       ...
       (COALESCE(att.c, 0) * 3
        ...
        + COALESCE(podium.first_c, 0) * 7
        + COALESCE(podium.second_c, 0) * 5
        + COALESCE(podium.third_c, 0) * 3
...
LEFT JOIN podium ON podium.user_id = a.id
LEFT JOIN cast_votes ON cast_votes.user_id = a.id
ORDER BY 16 DESC, a.last_name NULLS LAST, a.full_name;
$function$;
