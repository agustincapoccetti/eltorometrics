...
import { LIBRARY_CATEGORIES, categoryLabel, categoryIcon, getThumbnail, deriveThumbnail, getEmbedUrl } from "@/lib/library";
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {filtered.map((it) => {
            ...
            <div key={it.id} className="border border-border flex flex-col overflow-hidden">
              {thumb ? (
                <button type="button" onClick={() => embed ? setPreview(it) : window.open(it.url, "_blank", "noopener,noreferrer")} className="relative block aspect-video bg-muted overflow-hidden group text-left">
              ...
              <div className="p-3 flex flex-col flex-1">
                ...
                <button type="button" onClick={() => embed ? setPreview(it) : window.open(it.url, "_blank", "noopener,noreferrer")} className="text-xs text-primary mt-2 inline-flex items-center gap-1 hover:underline text-left">
